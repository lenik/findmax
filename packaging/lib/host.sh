...
#
# Environment:
#   FINDMAX_SRCDIR       project root (default: packaging/../..)
#   FINDMAX_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
FINDMAX_SRCDIR=${FINDMAX_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$FINDMAX_SRCDIR/.config/findmax" "$HOME/.config/findmax"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${FINDMAX_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${FINDMAX_PACKAGING:-}")"
...
cmd_run() {
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $FINDMAX_SRCDIR/.config/findmax/$k.build-host" >&2
        echo "    $HOME/.config/findmax/$k.build-host" >&2
    done
    return 2
}
